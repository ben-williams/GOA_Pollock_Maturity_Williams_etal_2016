...
pollya %>% 
   mutate(Year=factor(Year), Age = factor(Age), hauls = factor(hauls)) -> pollya


#Create a new dataset for predictions
rdat <- data.frame(expand.grid(age=seq(1,10,1), year=as.numeric(levels(pollya$Year)), dum=0), latitude=median(pollya$latitude), longitude=median(pollya$longitude), hauls='2013.4')
rdat$Year <- factor(rdat$year)
...
r <- predict(base,rdat, se=T, type='response')
rdat$base <- predict(base, rdat,type='response')
rdat$base.se2 <- r$se.fit^2

r <- predict(sp,rdat, se=T, type='response')
rdat$sp <- as.numeric(predict(sp, rdat,type='response'))
rdat$sp.se2 <- as.numeric(r$se.fit^2)
...
rdat %>% 
   group_by(age, year) %>% 
   summarise(base = mean(base), base.se = sqrt(sum(base.se2)/10),sp = mean(sp), sp.se = sqrt(sum(sp.se2)/10)) %>% 
   mutate(basell = base - 2*base.se,baseul = base + 2*base.se,
          spll = sp - 2*sp.se,spul = sp + 2*sp.se) -> mats

rdatar %>% 
   group_by(age, year) %>% 
   summarise(wt = mean(wt), wt.se = sqrt(sum(wt.se2)/10)) %>% 
   mutate(wtll = wt - 2*wt.se,wtul = wt + 2*wt.se) -> mats1

mats %>% 
   left_join(mats1) -> mats2
...
biomass %>% 
   ...
   filter(year>1983 , year!=1999 , year!=2011, year< 2014)-> abundance
...
alt <- data.frame(year = rep(c(1999,2011), each=3), base.ll=NA, base.ul=NA, sp.ll=NA, sp.ul=NA, wt.ll=NA, wt.ul=NA,variable =c('base','sp','wt' ), value=NA, ll=NA, ul=NA)

ac <- rbind(alt,ac)

tiff(filename = "./chapter_1/figs/Fig11.tiff", width = 5.2, height = 5.2, units = "in", res=600, compression="lzw")
...
ggplot(ac, aes(year, value, color=variable))+geom_line()+geom_ribbon(aes(ymin= ll, ymax = ul, fill = variable), alpha= .2, color = NA)+
   ...
   scale_x_continuous(breaks=1980:2013, labels=c(1980, rep('',4),1985, rep('',4),1990, rep('',4),1995, rep('',4),2000, rep('',4),2005, rep('',4),2010, rep('',3)))

dev.off() 
#ggsave("./chapter_1/figs/Fig11.tiff", dpi=300, height=6, width=6, units="in")
...
abundance %>% 
   ...
   summarise(spmean = mean(sp.diff), spmin = min(sp.diff), spmax= max(sp.diff), spsd = sd(sp.diff),wt.d = mean(wt.diff))
